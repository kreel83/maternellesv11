...
<div class="padding-top padding-bottom about-bottom">
	<div class="container">
		...
		<p>{{ env('APP_URL') }} se réserve le droit de modifier unilatéralement et à tout moment le
		contenu des présentes CGU.</p>

		<h4>Article 1 : Les mentions légales</h4>

		<p>L’édition et la direction de la publication du site {{ env('APP_URL') }} est assurée par Les
		...
		<p>L'hébergeur du site {{ env('APP_URL') }} est la société Planethoster, dont le siège social
		est situé au 4416 Louis-B.-Mayer Laval, Québec Canada H7P 0G1, avec le numéro de téléphone :
		+33176604143.</p>

		<h4>ARTICLE 2 : Accès au site</h4>

		<p>Le site {{ env('APP_URL') }} permet à l'Utilisateur un accès gratuit aux services suivants :
		...
		la responsabilité de {{ env('APP_URL') }}. Dans ces cas, l’Utilisateur accepte ainsi ne pas
		...
		<p>Les sources des informations diffusées sur le site {{ env('APP_URL') }} sont réputées
		...
		Malgré des mises à jour régulières, le site {{ env('APP_URL') }} ne peut être tenu
		...
		<p>Le site {{ env('APP_URL') }} ne peut être tenu pour responsable d’éventuels virus qui
		...
		liens, il sortira du site {{ env('APP_URL') }}. Ce dernier n’a pas de contrôle sur les pages
		...
		{{ env('APP_URL') }}. Les cookies ne contiennent pas d’information personnelle et ne
		...
		{{ env('APP_URL') }}. En naviguant sur le site, L’Utilisateur les accepte.</p>
