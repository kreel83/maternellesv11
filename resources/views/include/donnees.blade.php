{{-- <p>La présente politique de confidentialité est à jour au 01 janvier 2023.</p> --}}

{{-- https://www.index-education.com/fr/politique-de-protection-des-donnees.php --}}

<p>ET BAM Solutions, SAS au capital de 1000 €, Siège social : 33 rue Claire Joie - 83200 Toulon, en qualité de responsable de traitement a désigné un Délégué à la Protection des Données (DPO).</p>
<p>Vous pouvez contacter notre DPO pour tout renseignement complémentaire ou pour exercer vos droits en écrivant à l’adresse suivante :</p>
<p>SAS ET BAM Solutions - DPO - 33 rue Claire Joie - 83200 Toulon ou par email à : donneepersonnelle@etbamsolutions.com.</p>

<h5 class="mt-3">Pourquoi nous collectons vos données personnelles ?</h5>

<p>Lorsque ET BAM Solutions collecte vos données personnelles, elle le fait pour des finalités spécifiques, légitimes, déterminées et transparentes.</p>

<p>ET BAM Solutions est amenée à traiter vos données sur les fondements et dans les finalités suivantes :</p>
<br>
<p>Fondement légal de l’exécution de nos obligations contractuelles</p>
<p>Les données recueillies par ET BAM Solutions ont pour finalité l'utilisation de notre logiciel.</p>
<br>
{{-- <p>Fondement légal de votre consentement</p>
<p>Les données recueillies par ET BAM Solutions ont pour finalité l’organisation de Jeu-concours, Prospection commerciale/marketing (segmentation, publicités ciblées, newsletter, etc) lorsque le traitement n’est pas fondé sur la base légale de l’intérêt légitime, Dépose des cookies à des fins de proposition de publicités et contenus standards et personnalisés, mesure de performance des publicités et du contenu, mesure d'audience, études de marché et développement de produit. ET BAM Solutions utilise une technologie de recommandation et de personnalisation afin d'améliorer votre expérience en vous proposant des contenus et des offres les plus adaptés à vos centres d'intérêt, via le navigateur nous recueillons vos données de géolocalisation. La technologie de recommandation se base sur vos données de navigation et/ou sur votre historique de location et vos demandes de devis.</p>
<br>
<p>Fondement légal de l’intérêt légitime poursuivi par ET BAM Solutions</p>
<p>Les données recueillies par ET BAM Solutions ont pour finalité le contrôle d’éventuelles fraudes (y compris le respect des conditions de location, le vol, le détournement), la gestion des avis produits, gestion des impayés, l’analyse des statistiques, les opérations techniques (normalisation des adresses postales, enrichissement, déduplication), le suivi de la qualité de nos services et pourront aussi être utilisées à des fins de prospection commerciale/marketing lorsque vous êtes un professionnel ou que vous avez déjà acheté un bien ou service analogue (segmentation, publicités ciblées, newsletter). ET BAM Solutions procède également au dépôt de cookies ayant pour finalité d’assurer le bon fonctionnement et l’amélioration permanente de nos sites internet et de leurs fonctionnalités lorsque votre consentement n’est pas requis.</p>
<br> --}}
<p>Fondement légal du respect d’une obligation légale</p>
<p>Les données recueillies par ET BAM Solutions ont pour finalité de répondre aux obligations légales auxquelles ET BAM Solutions peut être soumise, notamment, s’agissant de la gestion des infractions, du respect des obligations de conservation en application de la réglementation, de la gestion des demandes relatives aux droits des personnes.</p>

<h5 class="mt-3">Comment nous collectons vos données personnelles ?</h5>

<p>Nous collectons vos données sur notre site par le biais de nos formulaires en ligne, ainsi que lors de la création de votre espace personnel en ligne ou de la modification de vos informations sur votre espace personnel.</p>

<h5 class="mt-3">Combien de temps sont conservées vos données ?</h5>

<p>ET BAM Solutions conserve vos données pour une durée strictement nécessaire à la réalisation des finalités.</p>
<p>Les données nécessaires à la poursuite des finalités visées, à la justification de nos prestations, à la préservation de nos droits, et au respect des obligations légales nous incombant sont conservées durant le temps nécessaire à la poursuite de ces finalités, aux obligations nous incombant (telle que la conservation des documents comptables et des pièces justificatives au titre de l’article L 123-22 du code de commerce), et aux règles de prescriptions applicables en fonction de la nature des droits concernés.</p>

<h5 class="mt-3">Sécurité</h5>

<p>Lors de votre navigation sur nos sites internet et notre application mobile les données sont chiffrées via un protocole https.</p>
<p>Concernant votre espace client, vous devez respecter les consignes de création de mot de passe indiquées. En cas d’oubli ou perte de mot de passe vous devrez suivre les instructions relatives à la réinitialisation de celui-ci.</p>
<p>Le stockage de vos données est effectué sur des serveurs sécurisés, situés en France.</p>

<h5 class="mt-3">Destinataires de vos données</h5>
<p>ET BAM Solutions est le seul destinataire de vos données. Aucune information personnelle n'est cédée à des tiers en dehors de procédures légales.</p>
<p>Par ailleurs, ET BAM Solutions ne collecte aucune « données sensibles ».</p>

<h5 class="mt-3">Vos droits Informatique et Libertés</h5>

<p>Conformément à la loi Informatique et Libertés du 06 janvier 1978 modifiée, vous disposez d’un droit d'accès, de rectification, d’effacement, de limitation, d’opposition, à la portabilité aux informations vous concernant. Vous disposez du droit de définir des directives relatives au sort de vos données personnelles et à la manière dont vous souhaitez que vos droits soient exercés après votre décès. Vous disposez également du droit d’introduire une réclamation auprès de la Cnil.</p>

<h5 class="mt-3">Exercer vos droits Informatique et Libertés</h5>

<p>Vous pouvez exercer vos droits en envoyant votre demande à : SAS ET BAM Solutions - DPO - 33 rue Claire Joie - 83200 Toulon ou par email à : donneepersonnelle@etbamsolutions.com. Une réponse vous sera alors adressée dans un délai de 1 mois suivant la réception de la demande.</p>

<p>Toute demande d'accès, de rectification, d’effacement, de limitation, d’opposition, à la portabilité portant sur vos données nécessite la présentation d’une pièce d’identité par envoi électronique ou courrier. Sans cette pièce, pour la protection de vos informations personnelles, il nous est impossible de vous les divulguer ou de les modifier.</p>

<p>Toutefois, lorsque votre demande de suppression porte sur votre compte sur {{ config('app.url') }}, et que l’adresse email du compte à supprimer correspond à l’adresse email avec laquelle vous nous écrivez, il n’est pas nécessaire de nous fournir une pièce d’identité. Vous pouvez alors nous envoyer simplement un email à donneepersonnelle@etbamsolutions.com. Si l’adresse email avec laquelle vous nous écrivez ne correspondant à l’adresse email du compte à supprimer, vous devrez nous transmettre par voie électronique ou courrier, une copie de votre pièce d’identité.</p>

<p>Nous précisons que certaines informations vous concernant ne peuvent, pour des raisons légales, être modifiées ou supprimées de nos bases de données. ET BAM Solutions pourra également démontrer, le cas échéant, qu’il existe des motifs légitimes et impérieux pour poursuivre le traitement qui prévalent sur les droits et les libertés de la personne concernée.</p>

<p>En cas de décès qui serait porté à notre connaissance, nous vous informons que vos données seront supprimées, sauf nécessité de conservation pendant une durée déterminée pour des motifs tenant à nos obligations légales et réglementaires et/ou aux délais légaux de prescription, et après le cas échéant avoir été communiquées à un tiers éventuellement désigné par vos soins.</p>

<h5 class="mt-3">Description du traitement</h5>
<br>
<p><strong>1. Finalités : Objet du ou des traitement(s)</strong></p>
<p>Finalité principale :</p>
<p>- Logiciel</p>
<p>Evaluation des élèves et conception du cahier de réussites.</p>
<p>- Application mobile (Compagnon)</p>
<p>Evaluation des élèves.</p>

<br>
<p><strong>2. Nature des opérations réalisées sur les données</strong></p>
<table class="table table-striped">
    <tbody><tr>
    <td></td>
    <td><p align="center">Cochez </p></td>
    <td><p align="center">Commentaires / précisions</p></td>
    </tr>
    <tr>
    <td><p>Collecte</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Informations des élèves et de leurs responsables légaux sur leurs événements de vie scolaire.</p></td>
    </tr>
    <tr>
    <td><p>Enregistrement</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Enregistrement de l'ensemble des données.</p></td>
    </tr>
    <tr>
    <td><p>Organisation / classement / structuration</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Classement des données.</p></td>
    </tr>
    <tr>
    <td><p>Conservation</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Conservation des données dans une base au format propriétaire.</p></td>
    </tr>
    <tr>
    <td><p>Adaptation ou modification</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Toute donnée du logiciel en fonction des profils et des droits.</p></td>
    </tr>
    <tr>
    <td><p>Extraction</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
    <td></td>
    </tr>
    <tr>
    <td><p>Export</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Export des données vers le cahier de réussites.</p></td>
    </tr>
    <tr>
    <td><p>Consultation</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Toute donnée du logiciel en fonction des profils et des droits.</p></td>
    </tr>
    <tr>
    <td><p>Utilisation</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Toute donnée du Logiciel en fonction des profils et des droits dans le cadre des finalités.</p></td>
    </tr>
    <tr>
    <td><p>Communication par transmission</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Transmission  des données suivantes :<br>
    - Lien de téléchargement du cahier de réussites à destination des responsables légaux.
    </p></td>
    </tr>
    <tr>
    <td><p>Diffusion ou toute autre forme de mise à disposition</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Mise à disposition des données pour les enseignants via l'application.</p></td>
    </tr>
    <tr>
    <td><p>Rapprochement ou l'interconnexion</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
    <td><p></p></td>
    </tr>
    <tr>
    <td><p>Effacement ou destruction</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Dans le respect des obligations légales de conservation des données&nbsp;:<br>
    Archivage annuel.</p></td>
    </tr>
    <tr>
    <td><p>Impression</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Impression de toute donnée du logiciel.</p></td>
    </tr>
    <tr>
    <td><p>Saisie</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Toute donnée du logiciel, par exemple&nbsp;:<br>
    - Informations des élèves et de leurs responsables légaux,<br>
    - Informations pédagogiques (évaluations) par l'équipe éducative</p></td>
    </tr>
    <tr>
    <td><p>Contrôle</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
    <td><p></p></td>
    </tr>
    <tr>
    <td><p>Archivage</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
    <td><p>Dans le respect des obligations légales d'archivage.<br>
    Archivage annuel.</p></td>
    </tr>
    <tr>
    <td><p>Autres</p></td>
    <td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
    <td></td>
    </tr>
    </tbody>
</table>

<br>

<p><strong>3. Catégories de données à caractère personnel traitées</strong></p>
<p><u>Application {{ config('app.name') }}</u></p>
<table class="table table-striped">
<tbody><tr>
<td></td>
<td><p>Cochez </p></td>
<td><p>Description</p></td>
</tr>
<tr>
<td><p>Etat civil, identité, données d'identification, images…</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
<td><p><u>Élèves&nbsp;:</u><br>
Civilité, nom, prénoms, date de naissance.<br>
<u>Responsables légaux&nbsp;:</u><br>
Adresse électronique.<br>
<u>Professeurs et personnels&nbsp;:</u><br>
Civilité, nom, prénoms, adresse électronique.<br>
</p></td>
</tr>
<tr>
<td><p>Vie personnelle (habitudes de vie, situation familiale, etc.)</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
<td><p><u>Élèves&nbsp;:</u><br>
Situation de handicap.</p></td>
</tr>
<tr>
<td><p>Vie scolaire et/ou vie professionnelle (CV, formation professionnelles, distinction et diplômes)</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
<td><p><u>Élèves&nbsp;:</u><br>
Situation scolaire actuelle et précédente, statut.<br>
Evaluation des compétences.</p></td>
</tr>
<tr>
<td><p>Informations d'ordre économique et financier (revenus, situation financière, situation fiscale, etc.)</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td><p></p></td>
</tr>
<tr>
<td><p>Données bancaires (n° compte, RIB, IBAN, n° CB, etc.)</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données de connexion (adresse IP, identifiants dans des logs, etc.) </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td><p></p></td>
</tr>
<tr>
<td><p>Données de localisation (déplacements, données GPS, GSM, etc.)</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant l'origine raciale ou ethnique </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant les opinions politiques </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant les convictions religieuses ou philosophiques&nbsp; </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant l'appartenance syndicale </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données génétiques </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données biométriques aux fins d'identifier une personne physique de manière unique </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Numéro de sécurité sociale </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données concernant la santé </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td><p></p></td>
</tr>
<tr>
<td><p>Données concernant la vie sexuelle ou l'orientation sexuelle</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données relatives à des condamnations pénales ou infractions </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Données relatives aux mineurs </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
<tr>
<td><p>Autres données </p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square"></i> </p></td>
<td></td>
</tr>
</tbody>
</table>
<br>
<p><u>Application Mobile (Compagnon)</u></p>
<table class="table table-striped">
<tbody><tr>
<td></td>
<td><p>Cochez </p></td>
<td><p>Description</p></td>
</tr>
<tr>
<td><p>Données    d'identification</p></td>
<td style="text-align:center;"><p><i class="fa-regular fa-square-check"></i> </p></td>
<td><p>Adresse du serveur {{ config('app.name') }}, identifiants des comptes Utilisateur, jetons de connexion.</p></td>
</tr>
</tbody>
</table>

<br>

<p><strong>4. Durée de conservation des données</strong></p>
<table class="table table-striped">
<tbody><tr>
<td></td>
<td><p align="center">Durée de conservation<br>
base active (production) </p></td>
<td><p align="center">Durée de conservation<br>
archivage</p></td>
</tr>
<tr>
<td><p>Etat civil, identité, données d'identification, images…</p></td>
<td><p>Les données d'état-civil peuvent être conservées maximum trois ans à partir de la date de recueil de ces informations.<br>
Applications mobiles&nbsp;:<br>
Les données sont conservées tant que le compte est présent dans l’application mobile.</p></td>
<td><p>Les années antérieures sont conservées en archivage intermédiaire 5 années maximum.</p></td>
</tr>
<tr>
<td><p>Vie personnelle (habitudes de vie, situation familiale, etc.)</p></td>
<td><p>Les données sont conservées pour une durée correspondant à l'année scolaire.</p></td>
<td><p>Les années antérieures sont conservées en archivage intermédiaire 5 années maximum.</p></td>
</tr>
<tr>
<td><p>Vie scolaire et/ou vie professionnelle (CV, formation professionnelles, distinction et diplômes)</p></td>
<td><p>Les données sont conservées pour une durée correspondant à l'année scolaire.</p></td>
<td><p>Les années antérieures sont conservées en archivage intermédiaire 5 années maximum.</p></td>
</tr>
<tr>
<td><p>Informations d'ordre économique et financier (revenus, situation financière, situation fiscale, etc.)</p></td>
<td><p></p></td>
<td><p></p></td>
</tr>
<tr>
<td><p>Données bancaires (n° compte, RIB, IBAN, n° CB, etc.)</p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données de connexion (adresse IP, identifiants dans des logs, etc.) </p></td>
<td><p></p></td>
<td><p></p></td>
</tr>
<tr>
<td><p>Dnnées de localisation (déplacements, données GPS, GSM, etc.)</p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant l'origine raciale ou ethnique </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant les opinions politiques </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant les convictions religieuses ou philosophiques&nbsp; </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données révélant l'appartenance syndicale </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données génétiques </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données biométriques aux fins d'identifier une personne physique de manière unique </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Numéro de sécurité sociale </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données concernant la santé </p></td>
<td><p></p></td>
<td><p></p></td>
</tr>
<tr>
<td><p>Données concernant la vie sexuelle ou l'orientation sexuelle</p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données relatives à des condamnations pénales ou infractions </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Données relatives aux mineurs </p></td>
<td></td>
<td></td>
</tr>
<tr>
<td><p>Autres données </p></td>
<td></td>
<td></td>
</tr>
</tbody>
</table>
<br>
<p><strong>5. Catégories de personnes concernées</strong></p>
<p>Professeurs et personnels de l'établissement</p>
<p>Élèves de l'établissement</p>
<p>Responsables légaux des élèves de l'établissement</p>