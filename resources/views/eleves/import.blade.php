    ...
    <div class="d-none d-xxl-block position-relative gx-0 navigateur" style="width: 100%; height: 100vh">
        ...
        <div class="col-md-6" style="color: var(--main-color); font-weight: bolder; width: 474px;padding: 16px;height: 617px; border: 1px solid var(--main-color); border-radius: 8px">
            ...
            <table class="table table-bordered">
                ...
                <tr class="classe" data-classe="{{$classe->id}}">
                    <td class="ps-5">    
                        ...
                        <span class="{{ config('app.custom.app_demo') && Auth::id() == config('app.custom.app_demo_user') ? 'blur' : null}}">{{$classe->name}}</span> {{$classe->prenom}} 
                    </td>
                </tr>
                @endforeach
            </table>
            ...
            <div class="offset-md-1 col-md-5" style="color: var(--main-color); font-weight: bolder; ; background-color: white">
                <div class="my-2 fs-6">Les élèves de mon école<span class="help" data-location="eleve.import.eleves"><i class="fa-light fa-message-question"></i></span></div>
                ...
                <table class="table table-bordered">
                    ...
                    <tr class="enfant" data-classe="{{$enfant->classe_n1_id}}" data-texte="{{ $enfant->nom}} {{ $enfant->prenom}}">
                        ...
                        <td>
                            <span class="{{ config('app.custom.app_demo') && Auth::id() == config('app.custom.app_demo_user') ? 'blur' : null}}">{{$enfant->nom}}</span> {{ $enfant->prenom}} <span class="psmsgs">{{$enfant->psmsgs}}</span>
                        </td>
                    </tr>
                    @endforeach
                </table>
